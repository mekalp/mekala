...
//I can use the above code for the hotels to display from the latest date//
...
let hotel = await Hotel.findById(req.params.hotelId).select("-image.data").populate("postedBy","_id name").exec()
res.json(hotel)
...
let result = await Hotel.find({from: {$gte: new Date(), to:{site: to}}, location, bed}).select("-image.data").exec()
res.json(result)
